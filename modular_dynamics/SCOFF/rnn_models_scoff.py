...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5,
                 ...
                 num_rules = 0, rule_time_steps = 0, perm_inv = False):
    ...
    def init_blocks(self):
        ...
        if True:
            if True:
                bc_lst.append(BlocksCore(ninp, nhid, 1, num_blocks, topk, memorytopk, step_att, num_modules_read_input, inp_heads, do_gru=use_gru,
                                         do_rel=do_rel, device=device, n_templates=n_templates, share_inp=share_inp, share_comm=share_comm, memory_mlp=memory_mlp,
        ...
        if True:
            if self.perm_inv:
                ...
                self.pma = PMA(self.block_size, 4, 1)
                self.decoder = nn.Linear(self.block_size, self.nhid)
            if tie_weights:
                print('tying weights!')
                if self.nhid != ninp:
    ...
    def forward(self, input, hidden):
        ...
        if True:
            ...
            for idx_layer in range(0, self.nlayers):
                ...
                for idx_step in range(input.shape[0]):
                    hx, cx, mask, bmask, temp_attn = self.bc_lst[idx_layer](layer_input[idx_step], hx, cx, idx_step,
                                                                 do_print=do_print)
                    output.append(hx)
                    masklst.append(mask)
        ...
        return dec.view(output.size(0), output.size(1), dec.size(1)), \
               hidden, extra_loss, block_mask, template_attn
