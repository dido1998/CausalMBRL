...
movement=$5
edge=$6
python data_gen/env.py --env-id ColorChangingTimeRL-$num_obj-$num_colors-$movement-$edge-$max_steps-v0 --fname data/ColorChangingTimeRL_$num_obj-$num_colors-$edge-$max_steps-$movement-train-$graph.h5  --num-episodes 1000 --episode-length 50 --seed 1 --save_graph --save_graph_location data/ColorChangingTimeRL_$num_obj-$num_colors-$edge-$max_steps-$movement-train-graph-$graph --graph $graph
