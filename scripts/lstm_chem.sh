...
if [ -z "$9" ]
then
	save_folder=lstm-$num_obj-$num_colors-$max_steps-$movement-$seed-$contrastive_loss-$time
	...
	save_folder=lstm-$num_obj-$num_colors-$edge-$max_steps-$movement-$seed-$contrastive_loss-$time
...
then
	...
	--encoder "medium" --name  "lstm_$num_obj-$size-$num_colors-$seed"   \
	...
	--epochs 100 --pretrain-epochs 100 --finetune-epochs 100 --predict-diff  --contrastive | tee -a "$save_folder_/train.log"
	...
	--valid-dataset $dataset3 \
	--save-folder $save_folder_ --batch-size 32 --seed $idx \
	--epochs 100 --pretrain-epochs 100 --finetune-epochs 100 --predict-diff  | tee -a "$save_folder_/train.log"
fi
