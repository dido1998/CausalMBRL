...
if [ -z "$9" ]
then
	save_folder=latest_models/gnn-$num_obj-$num_colors-$max_steps-$movement-$seed-$contrastive_loss-$time
	...
	save_folder=latest_models/gnn-$num_obj-$num_colors-$edge-$max_steps-$movement-$seed-$contrastive_loss-$time
...
then
	...
	--encoder "medium" --name  "gnn_$num_obj-$size-$num_colors-$seed"   \
	...
	--epochs 100 --pretrain-epochs 100 --predict-diff --gnn --contrastive | tee -a "$save_folder_/train.log"
	...
	--valid-dataset $dataset3 \
	--save-folder $save_folder_ --batch-size 512 --seed $idx \
	--epochs 100 --pretrain-epochs 100 --predict-diff --gnn | tee -a "$save_folder_/train.log"
fi
