...
if [ -z "$9" ]
then
	save_folder=latest_models/modular-$num_obj-$num_colors-$max_steps-$movement-$seed-$contrastive_loss-$time
	...
	save_folder=latest_models/modular-$num_obj-$num_colors-$edge-$max_steps-$movement-$seed-$contrastive_loss-$time
...
then
	...
	--encoder "medium" --name  "modular_$num_obj-$size-$num_colors-$seed"   \
	...
	--epochs 100 --pretrain-epochs 100 --predict-diff --modular --contrastive | tee -a "$save_folder_/train.log"
else
    save_folder_="$save_folder-$idx"
  	python train_baselines.py --dataset $dataset1 \
	--encoder "medium" --name  "modular_$num_obj-$size-$num_colors-$seed"   \
	...
	--valid-dataset $dataset3 \
	--save-folder $save_folder_ --batch-size 512 --seed $idx \
	--epochs 100 --pretrain-epochs 100 --predict-diff --modular | tee -a "$save_folder_/train.log"
